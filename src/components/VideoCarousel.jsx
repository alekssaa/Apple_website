...
const VideoCarousel = () => {
    ...
    /// video pocinje sa radom samo kada imamo loadedData pokrenucemo je sa funkcijom 
      ...
      <>
              ...
              {hightlightsSlides.map((list, i) => (
                  <div key={list.id} id="slider" className="sm:pr-20 pr-10">
                      <div className="video-carousel_container">
                          <div className="w-full h-full overflow-hidden rounded-3xl flex-center  bg-black">
                              <video
                                  id="video"
          ...
          <div className="relative flex-center mt-10">
              <div className="flex-center py-5 px-7 bg-gray-300 backdrop-blur rounded-full">
                  {videoRef.current.map((_, i) => (<span key={i} ref={(el) => (videoDivRef.current[i] = el)}
                  className="mx-3 w-3 h-3 bg-gray-200 rounded-full relative cursor-pointer"
                  >
                      <span className="h-full rounded-full absolute w-ful" ref={(el) => (videoSpanRef.current[i] = el)}></span>
                  </span>))}
              </div>
